...
contract Game {
  ...
  event PlayerCoordinateAdded(address indexed player, Coordinate indexed coordinate);
  event ClosestPlayerUpdated(
    address indexed player, uint256 indexed distance, Coordinate indexed coordinate
  );
  ...
  function deposit() external payable onlyInPhase(GamePhase.REGISTRATION) {
    ...
    if (currentPhase != GamePhase.REGISTRATION) revert NotInPhase(currentPhase);
    if (msg.value != registrationFee) revert DepositAmountIncorrect(msg.value);
  ...
  function withdrawDeposit() external {
    ...
    if (
      currentPhase != GamePhase.REGISTRATION
        && !(currentPhase == GamePhase.COMPLETED && closestPlayer.playerAddress == address(0))
  ...
  function addPlayerCoordinate(int64 _x, int64 _y)
    external
    onlyInPhase(GamePhase.ACTIVE)
    onlyRegistered
  {
    Coordinate memory newCoordinate = Coordinate(_x, _y);
    playerCoordinates[msg.sender].push(newCoordinate);
    emit PlayerCoordinateAdded(msg.sender, newCoordinate);
  }

  function revealTreasureCoordinate(int64 _x, int64 _y) external {
    GamePhase currentPhase = getCurrentPhase();
    if (currentPhase != GamePhase.RESOLUTION || currentPhase != GamePhase.COMPLETED) {
      revert NotInPhase(currentPhase);
    }
    if (!verifyTreasureCoordinate(_x, _y)) revert InvalidTreasureCoordinate();
    treasureCoordinate.x = _x;
    treasureCoordinate.y = _y;
  }

  // Anyone can call this function to update the closestPlayer
  function updateClosestPlayer(address player, uint256 coordinateId) external {
    ...
    emit ClosestPlayerUpdated(
      closestPlayer.playerAddress, closestPlayer.distance, closestPlayer.coordinate
    );
  }

  function getCurrentPhase() public view returns (GamePhase) {
    if (block.timestamp < registrationEndTime) return GamePhase.REGISTRATION;
  ...
  function verifyTreasureCoordinate(int64 _x, int64 _y) internal view returns (bool) {
